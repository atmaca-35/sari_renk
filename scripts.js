document.addEventListener('DOMContentLoaded', async () => {
        ...
        Object.keys(clickableWords).forEach(key => {
            clickableWords[key] = clickableWords[key].map(group => {
                return group.map(item => {
                    // "specialWords" içindeki anahtarları yakalayıp yerine "[SPECIAL:anahtar]" yazalım
                    Object.entries(specialWords).forEach(([specialKey, specialValue]) => {
                        const regex = new RegExp(`\\b${specialKey}\\b`, 'gi');  // Anahtarı regex'e çevir
                        item = item.replace(regex, `[SPECIAL:${specialKey}]`);  // Anahtarları "[SPECIAL:anahtar]" formatına çevir
                    });

                    // Tırnak içindeki metinleri vurgula
                    item = item.replace(/“([^”]+)”/g, '<span class="g">“$1”</span>');

                    // Numaraları typeWords ile değiştir ve sarı span içine al
                    item = item.replace(/\b(\d+)\b/g, (match) => {  // \d+ kullanarak çok basamaklı sayıları da yakalayabilirsiniz
                        const wordType = typeWords[match];  // TypeWords'ten karşılık gelen kelimeyi al
                        if (wordType) {
                            return `<span class="y">${wordType}</span>`;  // Sarı span ile kelimeyi döndür
                        }
                        return match;  // Eğer eşleşme yoksa olduğu gibi bırak
                    });

                    // Son olarak, "[SPECIAL:anahtar]" formatındaki yerleri temizleyelim
                    item = item.replace(/\[SPECIAL:\S+\]/g, '');

                    return item;  // Her item üzerinde işlem yapıldıktan sonra döndür
                });
            });
        });
    }
    ...
    async function loadData() {
        try {
            ...
            typeWords = vocabularyData.typeWords || {}; // typeWords'u JSON'dan al

            dictionaryData = entryWords;

            wrapClickableWords(); // wrap fonksiyonunu çağır
